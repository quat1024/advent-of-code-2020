...
impl Board {
    fn from_string(input: String) -> Board {
        let tiles = input.lines().map(|line| -> Vec<Tile> {
    ...
    fn step(&mut self) -> bool {
        ...
        for row in 0..self.height {
            for col in 0..self.width {
                ...
                let neighborhood: Vec<Tile> = ADJACENCY.iter()
                ...
                let occupied_count = neighborhood.iter().filter(|&&t| t == Tile::OccupiedSeat).count();
    ...
    fn print(&self) {
        ...
        for row in 0..self.height {
            for col in 0..self.width {
                print!("{}", match self.tiles[row][col] {
                    Tile::EmptySeat => 'L',
                    Tile::Floor => '.',
                    Tile::OccupiedSeat => '#'
                });
